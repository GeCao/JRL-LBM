...
mus = [0.89e-3, 1.57e-3, 3.99e-3, 6.06e-3, 11.65e-3, 45.23e-3]
rhos = [1e3, 1.0239e3, 1.05329e3, 1.07819e3, 1.09824e3, 1.11614e3]
Ds = [7.85e-10, 5.64e-10, 3.33e-10, 1.52e-10, 0.648e-10, 0.22e-10]

Res = [rhos[j] * 100e-6 * 12.5e-6 / mus[j] for j in range(len(mus))]
Pes = [100e-6 * 12.5e-6 / Ds[j] for j in range(len(Ds))]
...
def main(
    ...
    prefix = f"_g{int(abs(gravity_strength_real))}"
    mkdir(f"{path}/{prefix}")
