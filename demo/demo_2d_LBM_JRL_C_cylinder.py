...
def main(
    ...
    prefix = f"_cylinder_g{int(abs(gravity_strength_real))}"
    ...
    C_flags[..., :-1, -1] = int(CellType.INFLOW_2) if is_negative else int(CellType.INFLOW_2)  # right
    ...
    # 1. Run balance
    C_flags[..., 0:walks_int, 0:Rg_radius] = int(CellType.OBSTACLE)
    ...
    for step in tqdm(range(total_steps)):
        ...
        dC_r = (rC[1:] - rC[:-1]) / r[:-1]
        new_inflow = ((dC_z + dC_r) * area_array).sum()
        ...
        dC_r = (rC[1:] - rC[:-1]) / r[:-1]
        new_current = ((dC_z + dC_r) * area_array).sum()
        current_record[0] = (
