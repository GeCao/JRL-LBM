...
def main(
    ...
    prefix = f"_45degree_g{int(abs(gravity_strength_real))}"
    mkdir(f"{path}/{prefix}")
    ...
    for step in tqdm(range(total_steps)):
        ...
        is_obs = phi_obs <= 0
        phi_obs_vel = torch.where(
            is_obs.repeat(1, dim, *([1] * dim)),
            torch.Tensor([0, vel_obs]).to(device).to(dtype).reshape(1, dim, *([1] * dim)).expand_as(vel),
            torch.zeros_like(vel),
        )
        phi_obs_vel[:, :, -1, :] = 0  # Upper boundary not count
        ...
        flags[is_obs] = int(CellType.OBSTACLE)
        vel = torch.where(
            is_obs.repeat(1, dim, *([1] * dim)),
            phi_obs_vel,
            vel,
